...
# Staff Endpoints
# Replace the existing @router.get("/staff/{restaurant_name}") endpoint
# in src/settings_and_integrations.py with this:
...
async def get_restaurant_staff(
    ...
    Returns unique combinations of employee name and position.
    """
    try:
        cur = db.cursor(cursor_factory=RealDictCursor)
        ...
        if current_user["role"] == "SUPER_ADMIN":
            ...
            cur.execute("""
                SELECT DISTINCT ON (e.employee_name, e.position) 
                    e.*, r.name as restaurant_name
                FROM employees_graphs e
                JOIN restaurants r ON e.restaurant_id = r.id
                WHERE r.name = %s AND r.active = true
                ORDER BY e.employee_name, e.position
            """, (restaurant_name,))
...
# Replace the existing @router.get("/staff/role-count/{restaurant_id}") endpoint
# in src/settings_and_integrations.py with this:

@router.get("/staff/role-count/{restaurant_id}")
def get_role_counts(restaurant_id: int, conn=Depends(get_db)):
    """
    Get count of unique employees per role for a specific restaurant.
    Only counts unique employee names (no duplicates).
    """
    try:
        cur = conn.cursor(cursor_factory=DictCursor)
        cur.execute("""
            SELECT position AS role, COUNT(DISTINCT employee_name) AS count
