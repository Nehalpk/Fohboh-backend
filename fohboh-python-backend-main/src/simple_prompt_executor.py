...
class SimplePromptExecutor:
    ...
    def match_prompt(self, user_text: str) -> Optional[Dict]:
        ...
        logger.info(f"ðŸ” Matching text: '{user_text_lower}'")
        ...
        if has_time_period:
            logger.info(f"â° Time period detected, prioritizing KPI prompts")
            for prompt in self.prompts:
                if prompt.get('category') == 'kpi':
                    for pattern in prompt['text_patterns']:
                        if pattern.lower() in user_text_lower:
                            logger.info(f"âœ… Matched KPI pattern '{pattern}' in prompt '{prompt['id']}'")
                            return prompt
        ...
        for prompt in self.prompts:
            ...
            for pattern in prompt['text_patterns']:
                if pattern.lower() in user_text_lower:
                    logger.info(f"âœ… Matched pattern '{pattern}' in prompt '{prompt['id']}'")
                    return prompt
        
        logger.info("âŒ No prompt matched")
        return None
    ...
    def _load_restaurant_data(self, restaurant_names: List[str], time_period: Optional[str] = None) -> Dict[str, pd.DataFrame]:
        ...
        """
        try:
            logger.info(f"ðŸª Loading data for restaurants: {restaurant_names}")
            logger.info(f"ðŸ“… Time period requested: {time_period or 'All time'}")
            ...
            if not restaurant_ids:
                logger.warning(f"âŒ No active restaurants found for names: {restaurant_names}")
                return {
                    'sales': pd.DataFrame(),
            ...
            logger.info(f"âœ… Found {len(restaurant_ids)} restaurant(s): {[r['name'] for r in restaurant_rows]}")
            ...
            if time_period:
                ...
                if days:
                    ...
                    logger.info(f"ðŸ“† Cutoff date calculated: {cutoff_date} ({days} days ago)")
                else:
                    logger.warning(f"âš ï¸ Unknown time period: {time_period}, loading all data")
            ...
            if date_info and date_info['total_records'] > 0:
                logger.info(f"ðŸ“Š Database date range: {date_info['min_date']} to {date_info['max_date']}")
                ...
                if cutoff_date and date_info['max_date'] and cutoff_date > date_info['max_date']:
                    logger.warning(f"âš ï¸ Requested cutoff ({cutoff_date}) is after latest data ({date_info['max_date']})")
                    logger.warning(f"   No data will match this filter! Using all available data instead.")
                    cutoff_date = None  # Fall back to all data
            else:
                logger.warning(f"âš ï¸ No sales data found in database for these restaurants")
            
            # ============ LOAD SALES DATA ============
            logger.info(f"ðŸ’° Loading sales data...")
            ...
            if not sales_df.empty:
                logger.info(f"âœ… Loaded {len(sales_df)} sales records")
                ...
                logger.warning(f"âš ï¸ No sales data loaded")
            
            # ============ LOAD INVENTORY DATA ============
            logger.info(f"ðŸ“¦ Loading inventory data...")
            ...
            logger.info(f"{'âœ…' if not inventory_df.empty else 'âš ï¸'} Loaded {len(inventory_df)} inventory records")
            
            # ============ LOAD MENU DATA ============
            logger.info(f"ðŸ½ï¸ Loading menu data...")
            ...
            logger.info(f"{'âœ…' if not menu_df.empty else 'âš ï¸'} Loaded {len(menu_df)} menu records")
            
            # ============ LOAD LABOR DATA ============
            logger.info(f"ðŸ‘· Loading labor data...")
            ...
            logger.info(f"{'âœ…' if not labor_df.empty else 'âš ï¸'} Loaded {len(labor_df)} labor records")
            ...
            # ============ SUMMARY ============
            logger.info(f"ðŸ“‹ Data loading summary:")
            ...
            logger.error(f"âŒ Error loading restaurant data: {e}", exc_info=True)
            return {
                'sales': pd.DataFrame(),
                'inventory': pd.DataFrame(),
                'menu': pd.DataFrame(),
    ...
    def _execute_kpi_calculation(
        ...
        try:
            ...
            logger.info(f"ðŸ”¢ Calculating KPI: {kpi_type}")
            logger.info(f"ðŸ“… Time period: {time_period or 'All time'}")
    ...
    def _format_kpi_results_for_ai(self, results: Dict, restaurant_names: List[str], time_period: Optional[str]) -> str:
        ...
        if 'splh' in kpis:
            kpi = kpis['splh']
            if kpi['value']:
                context += f"â±ï¸ **Sales per Labor Hour (SPLH)**: ${kpi['value']:,.2f}\n"
        ...
        if 'guest_count' in kpis:
            ...
            context += f"ðŸ‘¥ **Guest Count**: {kpi['value']:,}\n"
        ...
        if 'food_cost_pct' in kpis:
            ...
            if kpi['value']:
                context += f"ðŸ½ï¸ **Food Cost Percentage**: {kpi['value']:.2f}%\n"
        ...
        if 'inventory_turnover' in kpis:
            ...
            if kpi['value']:
                context += f"ðŸ”„ **Inventory Turnover Rate**: {kpi['value']:.2f}x\n"
        ...
        if 'low_inventory_warnings' in kpis:
            ...
            context += f"âš ï¸ **Low Inventory Warnings**: {kpi['total_low_items']} items\n"
            if kpi['value']:
        ...
        if 'labor_hours_by_role' in kpis:
            ...
            context += "ðŸ‘” **Labor Hours by Role**:\n"
        ...
        if 'overtime_pct' in kpis:
            ...
            context += f"â° **Overtime Percentage**: {kpi['value']:.2f}%\n"
        ...
        if 'popularity_vs_profitability' in kpis:
            ...
            context += "ðŸŽ¯ **Menu Engineering Analysis**:\n"
    ...
    def execute_query(
        ...
        logger.info(f"ðŸŽ¯ Matched prompt: {prompt['id']}")
        ...
        try:
            ...
            if prompt.get('requires_date', False):
                ...
                logger.info(f"ðŸ“… Executing query with date: {date}")
                ...
                logger.info(f"ðŸ“… Executing query with date range: {date_range[0]} to {date_range[1]}")
                ...
                logger.info(f"ðŸ”Ž Executing query with keyword: {keyword}")
                cur.execute(prompt['sql'], (restaurant_names, f'%{keyword}%'))
            else:
                logger.info(f"ðŸ” Executing query for restaurants: {restaurant_names}")
            ...
            logger.info(f"âœ… Formatted context for AI ({len(data_for_ai)} chars)")
            ...
            logger.error(f"âŒ Error executing query: {e}", exc_info=True)
            return {
                'matched': True,
                'query_executed': False,
                'error': str(e),
