...
class KPIValidationStatus(BaseModel):
    kpi_name: str
    kpi_number: int
    is_available: bool
...
class KPIAnalysisResponse(BaseModel):
    total_kpis_checked: int
    available_kpis: int
    kpi_status: Dict[str, KPIValidationStatus]
    summary: Dict[str, bool]

# Add this endpoint before the if __name__ == "__main__": line

@app.post("/analyze-kpis", response_model=KPIAnalysisResponse)
async def analyze_kpis(
    kpi_file: UploadFile = File(..., description="KPI JSON file"),
    current_user: dict = Depends(get_current_user)
):
    """
    Analyze KPI JSON file and return availability status for each KPI.
    
    Upload a JSON file containing KPI data to check which KPIs are available.
    
    Returns:
    - Total KPIs checked: 21
    - How many are available
    - Detailed status for each KPI
    - Simple true/false summary based on is_available
    """
    try:
        ...
        # Check if KPIs key exists
        if "kpis" not in data:
            raise HTTPException(
                status_code=400,
                detail="JSON must contain 'kpis' key"
            )
        
        kpis = data["kpis"]
        
        # Define KPI mapping with numbers
        kpi_mapping = {
            "total_sales": 1,
            "net_revenue": 2,
            "splh": 3,
            ...
            "avg_check": 5,
            "top_items": 6,
            "gross_profit_per_item": 7,
            "food_cost_pct": 8,
            "cogs_by_category": 9,  # Also affects 21
            "inventory_turnover": 10,
            "popularity_vs_profitability": 11,
            "labor_hours_by_role": 12,
            "overtime_pct": 13,
            "low_inventory_warnings": 14,
            "table_turn_time": 15,
            "kds_ticket_time": 16,
            "order_accuracy_rate": 17,
            "seasonal_sales": 18,
            "promo_roi": 19,
            "dish_waste_pct": 20,
            "total_cogs": 21  # Special case: true if cogs_by_category is true
        }
        ...
        # Analyze each KPI
        for kpi_key, kpi_number in kpi_mapping.items():
            # Special case for total_cogs - it's true if cogs_by_category exists
            if kpi_key == "total_cogs":
                cogs_data = kpis.get("cogs_by_category", {})
                is_available = bool(cogs_data)
                validation_passed = cogs_data.get("validation_passed", False) if cogs_data else False
                has_data = bool(cogs_data.get("total_cogs")) if cogs_data else False
            else:
                kpi_data_item = kpis.get(kpi_key, {})
                is_available = bool(kpi_data_item)
                validation_passed = kpi_data_item.get("validation_passed", False) if kpi_data_item else False
                
                # Check if there's actual data (value key exists and is not empty)
                if kpi_data_item:
                    value = kpi_data_item.get("value")
                    has_data = value is not None and value != [] and value != {}
                else:
                    has_data = False
            
            # Create status entry
            status = KPIValidationStatus(
                kpi_name=kpi_key,
            ...
            kpi_status[kpi_key] = status
            # Summary based ONLY on is_available
            summary[kpi_key] = is_available
            
            if is_available:
                available_count += 1
        
        logger.info(f"✅ KPI analysis completed for user {current_user['email']}: {available_count}/{len(kpi_mapping)} KPIs available")
        
        return KPIAnalysisResponse(
            total_kpis_checked=len(kpi_mapping),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze KPIs: {str(e)}"
        )
#------------------------------------------------------------------------
