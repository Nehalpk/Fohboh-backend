...
s3_client = boto3.client(
    ...
    aws_access_key_id='AKIA6G75DKGI2BZ2NGEJ',
    aws_secret_access_key='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'
)
...
class S3CSVProcessor:
    ...
    def _process_sales_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['Number of Items'] = pd.to_numeric(df['Number of Items'], errors='coerce')
    ...
    async def generate_kpis_from_s3(self, restaurant_name: str, current_user: dict, conn) -> Dict[str, Any]:
        ...
        try:
            ...
            restaurant_id = get_restaurant_id_by_name(restaurant_name)  # Replace with actual function to get restaurant ID
    ...
    def _process_menu_kpis(self, menu_df, results):
        ...
        try:
            # Menu revision tracker
            if self._check_required_columns(menu_df, ['menu_item', 'ingredient', 'created_at'], "menu revision tracker"):
                try:
                    menu_changes = menu_df.groupby(['menu_item', 'ingredient']).agg({"created_at": ["min", "max"]}).reset_index()
    ...
    def _process_sales_menu_kpis(self, sales_df, menu_df, results):
        ...
        try:
            ...
            try:
                ...
                # Update results
                results["Top Selling Items"] = [{"item": k, "units_sold": v["units"], "revenue": round(v["revenue"], 2)} for k, v in top_selling]
                results["Most Profitable Items"] = [{"item": k, "profit": round(v["revenue"] - v["cost"], 2)} for k, v in most_profitable]
                

                results["Top-Selling Menu Items"] = [{"item": k, "units_sold": v["units"], "revenue": round(v["revenue"], 2), "cost":round(v["cost"],2), "profit": round(v["revenue"] - v["cost"], 2) } for k, v in top_selling_menu_items]
                results["Underperformer Menu Items"] = [{"item": k, "units_sold": v["units"], "revenue": round(v["revenue"], 2), "cost":round(v["cost"],2), "profit": round(v["revenue"] - v["cost"], 2) } for k, v in under_performer_menu_items]
                ...
                for item, stats in item_stats.items():
                    ...
                    results["Least Profitable Menu Items"].append({"item": item, "profit_margin": round(margin, 2), "contribution_margin": round(profit, 2)})
                    results["Contribution Margin"].append({"item": item, "gross_profit": round(profit, 2)})
                    results["Food Cost %"].append({"item": item, "food_cost_percent": round(food_cost_pct * 100, 2), "cost": round(cost, 2), "price": round(revenue / sold if sold else 0, 2)})
                    results["Popularity vs Profitability"].append({"item": item, "sales_volume": sold, "contribution_margin": round(profit, 2)})
                    
                    if 'hours' in stats:
                        results["Time-of-Day Trends"].append({"item": item, "revenue_by_hour": {str(hour): round(val, 2) for hour, val in stats['hours'].items()}})
                    
                    results["Sales Ratio P-Mix"].append({"item": item, "sales_ratio": round((sold / total_sales) * 100, 2) if total_sales else 0, "contribution_margin": round(profit, 2)})
    ...
    def _process_sales_inventory_menu_kpis(self, sales_df, inventory_df, menu_df, results):
        ...
        try:
            ...
            if not self._check_required_columns(sales_df, ['total_amount', 'items_sold'], "sales-inventory-menu metrics"):
                return
                
            if not self._check_required_columns(inventory_df, ['ingredient', 'quantity', 'unit_cost'], "sales-inventory-menu metrics"):
                return
                
            if not self._check_required_columns(menu_df, ['menu_item', 'ingredient', 'amount', 'unit_cost'], "sales-inventory-menu metrics"):
                return
    ...
    def _calculate_kpis(self, sales_data, inventory_data, menu_data, employee_data):
        ...
        return self._calculate_kpis_resilient(sales_data, inventory_data, menu_data, employee_data, available_categories)
