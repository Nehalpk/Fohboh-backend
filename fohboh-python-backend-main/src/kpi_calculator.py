...
class KPICalculator:
    """
    Calculate KPIs based on formulas from FohBoh_KPI_Formulas_v1 Excel file
    """
    ...
    def calculate_total_sales(self, sales_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            total_sales = safe_float(sales_df[sales_col].sum())
    ...
    def calculate_net_revenue(self, sales_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            gross_sales = safe_float(sales_df[gross_col].sum())
            
            # Subtract comps, promos, voids (default to 0 if not present)
            comps = safe_float(sales_df['comps'].sum()) if 'comps' in sales_df.columns else 0
            promos = safe_float(sales_df['promos'].sum()) if 'promos' in sales_df.columns else 0
            voids = safe_float(sales_df['voids'].sum()) if 'voids' in sales_df.columns else 0
            ...
            if 'discount_percent' in sales_df.columns and gross_col in sales_df.columns:
                ...
                sales_df_copy['discount_amount'] = (
                    sales_df_copy[gross_col].apply(safe_float) * 
                    sales_df_copy['discount_percent'].apply(safe_float) / 100
                )
                discount_amount = safe_float(sales_df_copy['discount_amount'].sum())
    ...
    def calculate_splh(self, sales_df: pd.DataFrame, labor_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        Required: sales[net_revenue], labor[hours, is_salaried]
        """
        ...
        try:
            ...
            if 'is_salaried' in labor_df.columns:
                hourly_hours = safe_float(labor_df[labor_df['is_salaried'] == False]['hours_worked'].sum())
            elif 'hours_worked' in labor_df.columns:
                hourly_hours = safe_float(labor_df['hours_worked'].sum())
                self.warnings.append({
                    "kpi": kpi_name,
    ...
    def calculate_guest_count(self, sales_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            if 'covers' in sales_df.columns:
                guest_count = int(safe_float(sales_df['covers'].sum()))
    ...
    def calculate_top_items(self, sales_df: pd.DataFrame, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, row in sales_df.iterrows():
                ...
                total_amount = safe_float(row.get('total_amount', 0) or row.get('subtotal', 0))
        ...
        try:
            ...
            for _, row in menu_df.iterrows():
                ...
                if menu_item not in menu_cost_map:
                    menu_cost_map[menu_item] = 0.0
                menu_cost_map[menu_item] += amount * unit_cost
            
            # Calculate profit per item
            item_profits = []
            item_stats = defaultdict(lambda: {'revenue': 0.0, 'cost': 0.0, 'qty': 0})
            ...
            for _, row in sales_df.iterrows():
                ...
                revenue_per_item = total_amount / len(items) if items else 0.0
    ...
    def calculate_food_cost_pct(self, sales_df: pd.DataFrame, menu_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            food_cost_pct = self.safe_divide(total_cost, total_revenue) * 100 if total_revenue > 0 else None
            
            # Validation: 0 <= pct <= 100
            validation_passed = food_cost_pct is not None and 0 <= food_cost_pct <= 100
            
            if not validation_passed and food_cost_pct is not None:
                self.warnings.append({
                    "kpi": kpi_name,
    ...
    def calculate_inventory_turnover(self, sales_df: pd.DataFrame, menu_df: pd.DataFrame, 
        ...
        try:
            ...
            inventory_df['value'] = inventory_df['quantity'].apply(safe_float) * inventory_df['unit_cost'].apply(safe_float)
            avg_inventory_value = safe_float(inventory_df['value'].mean())
            ...
            if not validation_passed and turnover is not None:
                self.warnings.append({
                    "kpi": kpi_name,
    ...
    # ==================== KPI 11-20: Additional KPIs ====================
    # (Implementing simplified versions - can be enhanced based on available data)
    ...
    def calculate_low_inventory_warnings(self, inventory_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            low_items = inventory_df[
                inventory_df['quantity'].apply(safe_float) <= inventory_df['par_level'].apply(safe_float)
            ].copy()
            ...
            for _, row in low_items.iterrows():
                warnings.append({
                    "ingredient": row['ingredient'],
                    "current_quantity": safe_float(row['quantity']),
                    "par_level": safe_float(row['par_level']),
                    "shortage": safe_float(row['par_level']) - safe_float(row['quantity'])
                })
    ...
    def calculate_labor_hours_by_role(self, labor_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, row in hours_by_role.iterrows():
                result.append({
                    "role": row['position'],
                    "total_hours": round(safe_float(row['hours_worked']), 2)
                })
            ...
            return {
                "value": result,
                "total_hours": round(safe_float(labor_df['hours_worked'].sum()), 2),
                "validation_passed": True
            }
    ...
    def calculate_overtime_pct(self, labor_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate totals
            total_hours = safe_float(working_df['hours_worked'].sum())
            total_overtime_hours = safe_float(working_df['overtime_hours'].sum())
            ...
            if 'position' in working_df.columns:
                ...
                for _, row in role_groups.iterrows():
                    role_hours = safe_float(row['hours_worked'])
                    role_ot = safe_float(row['overtime_hours'])
    ...
    def filter_dataframe_by_date(self, df: pd.DataFrame, start_date, end_date, 
        ...
        try:
            ...
            logger.error(f"Error filtering DataFrame by date: {e}")
            return df
    ...
    def calculate_all_kpis(self, sales_df: pd.DataFrame, inventory_df: pd.DataFrame, 
        ...
        """
        Calculate all KPIs based on available data
        ...
        if time_period:
            start_date, end_date = self.get_date_range(time_period)
            if start_date and end_date:
                sales_df = self.filter_dataframe_by_date(sales_df, start_date, end_date, 'date')
                inventory_df = self.filter_dataframe_by_date(inventory_df, start_date, end_date, 'date')
                labor_df = self.filter_dataframe_by_date(labor_df, start_date, end_date, 'date')
...
def round_all_numbers(data, decimals=2):
    """
    Recursively round all numeric values in a nested dictionary/list structure.
    ...
    Returns:
        Data structure with all numbers rounded
    """
    import numpy as np
    
    if isinstance(data, dict):
        return {k: round_all_numbers(v, decimals) for k, v in data.items()}
    elif isinstance(data, list):
        return [round_all_numbers(item, decimals) for item in data]
