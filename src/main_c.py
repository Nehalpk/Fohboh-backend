...
class ChatHistory:
    ...
    def get_context(self, user_email: str) -> str:
        ...
        for conv in self.conversations:
            if conv.get('user_email') == user_email and conv.get('model') == "o":  # Use get() for safe access
                context += f"\nTime: {conv['timestamp']}\n"
    ...
    def clear_history(self,user_email: Optional[str]=None):
        if user_email:
            ...
            self.conversations = []
        self.save_to_file()
# You are Fohboh, a knowledgeable Restaurant Assistant. Your role is to help restaurant owners and managers with their questions and tasks.

FOHBOH_SYSTEM_PROMPT = """You are Fohboh, a knowledgeable Restaurant Assistant. Your role is strictly to assist restaurant owners and managers with general restaurant-related tasks and questions—such as operations, staffing, menus, customer service, and marketing. Do not respond to topics outside the restaurant industry.
...
When addressing labor cost calculations, assume there are 3 employees who have each worked for 6 hours. Use this as the foundational example to illustrate how labor cost is calculated unless otherwise specified.
...
class FileProcessor2:
    def __init__(self):
        self.api_key = "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"
        self.client = OpenAI(api_key=self.api_key)
        self.MAX_FILES = 3
        self.files = {}  # Changed from list to dict to store files by user_email
    ...
    async def process_files(self, files: List[UploadFile], user_email: str) -> dict:
        ...
        for file in files:
            ...
            try:
                ...
                if ext == '.pdf':
                    if self.process_pdf(file_bytes, file.filename, user_email):  # Pass user_email
                        processed_files["pdfs"] += 1
                    else:
                        processed_files["errors"].append(f"Failed to process PDF: {file.filename}")
                
                elif ext == '.csv':
                    if self.process_csv(file_bytes, file.filename, user_email):  # Pass user_email
    ...
    async def chat_from_transcript(self, transcript_text: str, request: TranscriptChatRequest, user_email: str) -> str:
    ...
    async def generate_streaming_response1(
    self,
    question: str,
    client_id: str,
    ...
    async def generate_response_with_embeddings(
        ...
        """Generate response using OpenAI API with optional embedding augmentation."""
        try:
            ...
            instructions = '''STRICT RESPONSE RULES:
1. Answer ONLY using data explicitly provided in the context above
2. NEVER create example scenarios or hypothetical calculations
3. NEVER say "you haven't provided data" or mention assumptions
4. Keep response under 150 words - be direct and concise
5. If specific numbers aren't available, provide conceptual guidance only
6. Include specific numeric values ONLY from the provided data
9. Be confident and factual - no apologetic language'''
            ...
            messages.append({
                ...
                "content": """Please format your response as follows:
                            1. Provide your answer based on the data directly, without prefixes like "FohBoh Response:"
                            2. Make sure to include specific numeric values from the data when applicable
                            3. Be concise but complete in your answer
                            4. Do not include any disclaimers or statements about the accuracy of your response""",
            ...
            try:
                ...
                for chunk in response_stream:
                    ...
                    # print(f"XXXXXXXXXXXXXXXXXXXXXXXXXXXX: {token}")
                    if token:
                        full_response += token
                        
                        # Here you could yield token for token-by-token processing or update UI
                        try:
                            # logger.warning(f"Attempting to broadcast token to user {user_id}")
                            # logger.debug(f"Active connections: {notification_manager.active_connections.keys()}")
                            
                            # Check if user has active connections
                            # if user_id in notification_manager.active_connections:
                            #     logger.info(f"Found {len(notification_manager.active_connections[user_id])} connections for user {user_id}")
                            # else:
                            ...
                            # await notification_manager.broadcast_to_user(user_id=user_id, message=message)
                ...
                # After full response is received
                # Remove prefix if exists
                if full_response.strip().startswith("FohBoh Response:"):
                    full_response = full_response.replace("FohBoh Response:", "", 1).strip()
    ...
    async def generate_streaming_response(
        ...
        try:
            ...
            if include_history:
                ...
                # Extract user info from past conversations
                for conv in user_history:
                    q = conv.get('query', {}).get('question', '').lower()
                    if "my name is" in q:
                        name_part = q.split("my name is")[1].strip()
            ...
            # Process embedding context
            embedding_context = ""
            if embedding_info:
                if embedding_info.get("multi_restaurant", False):
                    embedding_context += f"\n\nRelevant information from {embedding_info['restaurant_count']} restaurants:\n"
                    ...
                    for restaurant_data in embedding_info["restaurant_results"]:
                        ...
                        for category_data in restaurant_data["results"]:
                            ...
                            for file_data in category_data["files"]:
                                ...
                                for idx, result in enumerate(file_data["results"]):
                                    embedding_context += f"- {result['text_content']}\n"
                                    if idx >= 2:  # Limit to top 3 results
                                        break
            ...
            instructions = """Please format your response as follows:
                1. Provide a clear, concise answer using the available data
                2. Include specific numeric values when applicable
                3. Do not include data source citations
                4. CRITICAL: Do NOT include any follow-up questions"""
            ...
            logger.error(f"Streaming error: {str(e)}")
            await manager.send_message(f"Error: {str(e)}", client_id)
    async def generate_response(self, question: str, user_email: str, file_type: str = "all", 
                      file_index: int = -1, include_history: bool = True) -> str:
        ...
        try:
            ...
            if include_history and self.history.conversations:
                history_context = self.history.get_context(user_email)
                if history_context:
                    messages.append({
                        "role": "assistant",
